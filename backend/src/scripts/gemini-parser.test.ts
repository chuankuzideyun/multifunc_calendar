import { parseEmailContent, parseVoiceTranscript } from '../services/gemini';
...
async function runGeminiTests() {
  ...
  try {
    const events = await parseEmailContent(subject, body, emailDate);
  ...
  try {
    const result = await parseVoiceTranscript(voiceTranscript, refDate);
