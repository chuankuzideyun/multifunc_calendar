...
function runWeatherTests() {
    ...
    c1.temp = 24.9;
    assert((0, weather_1.checkRunningCriteria)(c1) === true, '24.9°C temp should meet criteria.');
    const c2 = createBaseValid();
    c2.temp = 25.0;
    assert((0, weather_1.checkRunningCriteria)(c2) === false, '25.0°C temp should fail running criteria (must be strictly < 25).');
    const c3 = createBaseValid();
    c3.temp = 25.1;
    assert((0, weather_1.checkRunningCriteria)(c3) === false, '25.1°C temp should fail running criteria.');
    ...
    p1.pop = 0.29;
    assert((0, weather_1.checkRunningCriteria)(p1) === true, '29% PoP should meet criteria.');
    const p2 = createBaseValid();
    p2.pop = 0.30;
    assert((0, weather_1.checkRunningCriteria)(p2) === false, '30% PoP should fail running criteria (must be strictly < 30%).');
    const p3 = createBaseValid();
    p3.pop = 0.31;
    assert((0, weather_1.checkRunningCriteria)(p3) === false, '31% PoP should fail running criteria.');
    // 3. Rain / Weather Conditions
    console.log('[Test] Verifying rain keyword matches...');
    const r1 = createBaseValid();
    r1.hasRain = false;
    assert((0, weather_1.checkRunningCriteria)(r1) === true, 'Condition with no rain should meet criteria.');
    const r2 = createBaseValid();
    r2.hasRain = true;
    assert((0, weather_1.checkRunningCriteria)(r2) === false, 'Condition with rain should fail criteria.');
